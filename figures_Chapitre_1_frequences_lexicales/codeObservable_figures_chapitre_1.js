...
Plot.plot({
  ...
  color: {
    legend: true,
    label: "Unigrammes",
    domain: ["journaliste","rédacteur",/*"chroniqueur","rédactrice","correspondant","reporter", "publiciste"*/],
    range:  ["blue","#FF5500",/*"green","orange","purple","brown", "grey"*/]
  },
  marks: [
    Plot.ruleY([0]),
    Plot.lineY(unigrams_data_a[0].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "blue"}),
    Plot.lineY(unigrams_data_a[1].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "#FF5500"}),
   // Plot.lineY(unigrams_data_a[2].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "green"}),
    //Plot.lineY(unigrams_data_a[3].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "orange"}),
  // Plot.lineY(unigrams_data_a[4].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "purple"}),
   // Plot.lineY(unigrams_data_a[5].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "brown"}),
    //Plot.lineY(unigrams_publiciste_data_a[0].items, {x: "date", y: "ppm", strokeWidth: 3, stroke: "grey"})
  ]
})
)}
...
Plot.plot({
  ...
  y: {
    label: "Fréquence'journaliste' (PPM)",
    grid: true,
    ticks: 20,
    domain: [0, 100]
  },
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.lineY(publiciste,  { x: "date", y: "ppm", strokeWidth: 3, stroke: "#A0A0A0" }),
    ...
    Plot.lineY(trend_publiciste,  { x: "date", y: "ppm", strokeWidth: 3, stroke: "#A0A0A0", strokeDasharray: "6,8" }),
...
function _66(Plot,periods,colors,yMax,chroniqueur,publiciste,boundaries,areaMarks,reporterSeries,corrLineMarks){return(
Plot.plot({
color: {
  legend: true,
  ...
  width: 1600,
  height: 800,
  x: {type: "time", tickFormat: "%Y", domain: [new Date("1870-01-01"), new Date("1990-12-31")], label: "Années"},
  y: {label: "Fréquence 'reporter' (PPM)", grid: true, ticks: 20, domain: [0, yMax]},
  marks: [
    Plot.ruleY([0]),
Plot.lineY(
  chroniqueur.map(d => ({...d, date: new Date(`${d.date}-01-01`), ppm: +d.ppm})),
  {x:"date", y:"ppm", stroke:"chroniqueur", strokeWidth:3}
),
Plot.lineY(publiciste.map(d => ({...d, date: new Date(d.date), ppm: +d.ppm})),  {x:"date", y:"ppm", stroke:"publiciste",  strokeWidth:3}),
    
    Plot.ruleX(boundaries, {strokeOpacity: 0.5}),
    ...areaMarks,
    Plot.lineY(reporterSeries, {x: "date", y: "ppm", stroke: "brown", strokeWidth: 3}),
    ...corrLineMarks,
    Plot.text(periods.map((per, i) => ({
      x: new Date((+per.start + +per.end)/2),
      y: yMax * 0.05,
      label: `${Math.round(per.p*1000)/10}%`,
      c: colors[i]
    })), {x: "x", y: "y", text: "label", fill: d => d.c, fontSize: 12})
  ]
})
)}
...
Plot.plot({
  ...
  marks: [
    ...
    // LIGNES CORRIGÉES INFO
    ...corrInfoMarks,

    // LIGNES CORRIGÉES OPINION
    ...corrOpinionMarks,
...
Plot.plot({
  ...
  y: {
    label: "Fréquence 'correspondant' (PPM)",
    grid: true,
    ticks: 20,
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["Plot","periods","colors","yMax","chroniqueur","publiciste","boundaries","areaMarks","reporterSeries","corrLineMarks"], _66);
